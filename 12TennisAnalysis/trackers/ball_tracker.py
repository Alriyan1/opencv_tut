...
class BallTracker:
    ...
    def draw_bboxes(self, video_frames, ball_detections):
        ...
        for frame, ball_dict in zip(video_frames, ball_detections):
            for track_id, bbox in ball_dict.items():
                x1, y1, x2, y2 = bbox
                cv2.putText(frame, f"Ball ID: {track_id}", (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,255,0), 2)
    ...
    def get_ball_shot_frames(self, ball_positions):
        ...
        #Convert the list into pandas dataframe
        df_ball_positions = pd.DataFrame(ball_positions, columns = ['x1', 'y1', 'x2', 'y2'])
        # center point/ mid point for y
        df_ball_positions['mid_y'] = (df_ball_positions['y1'] + df_ball_positions['y2']) / 2
        # center point/ mid point for y with rolling mean
        df_ball_positions['mid_y_rolling_mean'] = df_ball_positions['mid_y'].rolling(window=5, min_periods=1,
                                                                                     center=False).mean()
        # Caclulate the mid y rolling mean difference
